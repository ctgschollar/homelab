#!/usr/bin/env python3
"""
Homelab Agent (OpenAI) — main entrypoint.
...
async def run_repl(agent: HomelabAgent, config: AgentConfig, event_queue: asyncio.Queue, log_path: str) -> None:
    loop = asyncio.get_event_loop()
    console.print("[bold cyan]Homelab Agent (OpenAI)[/bold cyan] — type /quit to exit, /help for commands.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Homelab sysadmin agent (OpenAI-compatible API)")
