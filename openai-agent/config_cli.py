#!/usr/bin/env python3
"""
Config editor for the homelab OpenAI agent.
...
def cmd_pricing(args: list[str]) -> None:
    ...
    try:
        ...
        sys.exit(1)
    data = _load_raw()
    old_in = data["model"].get("input_cost_per_mtok", "unset")
    old_out = data["model"].get("output_cost_per_mtok", "unset")
    data["model"]["input_cost_per_mtok"] = input_cost
    data["model"]["output_cost_per_mtok"] = output_cost
    _save_raw(data)
