...
class AgentController:
    ...
    async def _cmd_model(self, text: str) -> str:
        ...
        if not sub:
            return f"Current model: `{self._config.model.name}`"
        if sub == "list":
    ...
    def _cmd_model_list(self) -> str:
        if not self._config.model.available_models:
            return "No models configured. Use `model add <name>` to add one."
        current = self._config.model.name
        lines = ["*Available models:*"]
        for m in self._config.model.available_models:
    ...
    async def _cmd_model_use(self, name: str) -> str:
        ...
        available = self._config.model.available_models
    ...
    async def _cmd_model_add(self, name: str) -> str:
        ...
        if name not in self._config.model.available_models:
            self._config.model.available_models.append(name)
            self._persist_available_models(self._config.model.available_models)
    ...
    def _persist_available_models(self, models: list[str]) -> None:
        try:
            with open(self._config_path) as f:
                data = yaml.safe_load(f) or {}
            data.setdefault("model", {})["available_models"] = models
            with open(self._config_path, "w") as f:
                yaml.dump(data, f, sort_keys=False, default_flow_style=False)
        except Exception as exc:
